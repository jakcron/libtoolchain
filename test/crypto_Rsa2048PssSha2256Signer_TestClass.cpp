...
crypto_Rsa2048PssSha2256Signer_TestClass::crypto_Rsa2048PssSha2256Signer_TestClass() :
	mTestTag("tc::crypto::Rsa2048PssSha2256Signer"),
	mTestResults()
{
}
...
void crypto_Rsa2048PssSha2256Signer_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kSignatureSize had value {:d} (expected {:d}", tc::crypto::Rsa2048PssSha2256Signer::kSignatureSize, kExpectedSignatureSize));
		}
...
void crypto_Rsa2048PssSha2256Signer_TestClass::test_UseUtilFuncSign()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			// test sign
			bool result = tc::crypto::SignRsa2048PssSha2256(signature.data(), test->message_digest.data(), tc::crypto::RsaPrivateKey(test->key_modulus.data(), test->key_modulus.size(), test->key_private_exponent.data(), test->key_private_exponent.size()));
			if (result == false)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: sign() returned false", test->test_name));
			}
...
void crypto_Rsa2048PssSha2256Signer_TestClass::test_UseUtilFuncVerify()
{
	...
	try
	{
		...
		{				
			// test verify
			bool result = tc::crypto::VerifyRsa2048PssSha2256(test->signature.data(), test->message_digest.data(), tc::crypto::RsaPublicKey(test->key_modulus.data(), test->key_modulus.size()));
			if (result == false)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: verify() returned false", test->test_name));
			}
