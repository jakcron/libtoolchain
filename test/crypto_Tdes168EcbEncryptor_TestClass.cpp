...
crypto_Tdes168EcbEncryptor_TestClass::crypto_Tdes168EcbEncryptor_TestClass() :
	mTestTag("tc::crypto::Tdes168EcbEncryptor"),
	mTestResults()
{
}
...
void crypto_Tdes168EcbEncryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d})", tc::crypto::Tdes168EcbEncryptor::kBlockSize, kExpectedBlockSize));
		}
		...
		{
			throw tc::TestException(fmt::format("kKeySize had value {:d} (expected {:d})", tc::crypto::Tdes168EcbEncryptor::kKeySize, kExpectedKeySize));
		}
...
void crypto_Tdes168EcbEncryptor_TestClass::test_UseUtilFuncEnc()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			for (size_t itr = 0; itr < test->iteration_count; itr++)
			{
				tc::crypto::EncryptTdes168Ecb(data.data(), data.data(), data.size(), test->key.data(), test->key.size());
			}
...
void crypto_Tdes168EcbEncryptor_TestClass::test_UseUtilFuncDec()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			for (size_t itr = 0; itr < test->iteration_count; itr++)
			{
				tc::crypto::DecryptTdes168Ecb(data.data(), data.data(), data.size(), test->key.data(), test->key.size());
			}
...
void crypto_Tdes168EcbEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
	...
	try
	{
		...
		// create data
		tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Tdes168EcbEncryptor::kBlockSize).pullData(0, tc::crypto::Tdes168EcbEncryptor::kBlockSize);
