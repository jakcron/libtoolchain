...
bn_string_TestClass::bn_string_TestClass() :
	mTestTag("tc::bn::string"),
	mTestResults()
{
}
...
void bn_string_TestClass::test_EncodedSizeVersusLogicalSize()
{
	...
	try 
	{
		// create bn::string with encoded size of 16 bytes (and implicit logical size of 16)
		{
			...
			{
				throw tc::TestException(fmt::format("sizeof(tc::bn::string<{:d}>) did not return ENCODED_SIZE ({:d})", kEncodedSize, kEncodedSize));
			}

			if (bn_string.max_size() != kEncodedSize)
			{
				throw tc::TestException(fmt::format("tc::bn::string<{:d}>::max_size() did not return LOGICAL_SIZE ({:d})", kEncodedSize, kEncodedSize));
			}
		}
		...
		// create bn::string with encoded size of 16 bytes and explicit logical size of 16
		{
			...
			if (sizeof(bn_string) != kEncodedSize)
			{
				throw tc::TestException(fmt::format("sizeof(tc::bn::string<{:d}, {:d}>) did not return ENCODED_SIZE ({:d})", kEncodedSize, kLogicalSize, kEncodedSize));
			}

			if (bn_string.max_size() != kLogicalSize)
			{
				throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::max_size() did not return LOGICAL_SIZE ({:d})", kEncodedSize, kLogicalSize, kLogicalSize));
			}
		}
...
void bn_string_TestClass::test_StringSizeNeverExceedsLogicalSize()
{
	...
	try 
	{
		...
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::size() did not respect LOGICAL_SIZE ({:d})", 20, 20, 20));
		}

		if (test.str_2020.data() != test.char_array.data())
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::data() did not return expected pointer)", 20, 20));
		}

		if (test.str_2020.decode() != base_string.substr(0, 20))
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::decode() returned \"{:s}\" (expected \"{:s}\")", 20, 20, test.str_2020.decode(), base_string.substr(0, 20)));
		}

		if (test.str_2010.size() != 10)
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::size() did not respect LOGICAL_SIZE ({:d})", 20, 10, 10));
		}

		if (test.str_2010.data() != test.char_array.data())
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::data() did not return expected pointer)", 20, 10));
		}

		if (test.str_2010.decode() != base_string.substr(0, 10))
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::decode() returned \"{:s}\" (expected \"{:s}\")", 20, 10, test.str_2010.decode(), base_string.substr(0, 10)));
		}

		if (test.str_2005.size() != 5)
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::size() did not respect LOGICAL_SIZE ({:d})", 20, 5, 5));
		}

		if (test.str_2005.data() != test.char_array.data())
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::data() did not return expected pointer)", 20, 05));
		}

		if (test.str_2005.decode() != base_string.substr(0, 5))
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::decode() returned \"{:s}\" (expected \"{:s}\")", 20, 5, test.str_2005.decode(), base_string.substr(0, 5)));
		}
...
void bn_string_TestClass::test_EncodeStringRespectsLogicalSize()
{
	...
	try 
	{
		...
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::encode() failed encode a string greater ({:d}) than the LOGICAL_SIZE", 20, 20, base_long_string.size()));
		}
		if (memcmp(test.char_array.data() + 20, empty_array.data(), 10) != 0)
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::encode() touched data after the LOGICAL_SIZE boundary", 20, 20));
		}

		test.str_2020.encode(base_short_string);
		if (test.str_2020.decode() != base_short_string)
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::encode() failed encode a string smaller ({:d}) than the LOGICAL_SIZE)", 20, 20, base_short_string.size()));
		}
		if (memcmp(test.char_array.data() + base_short_string.size(), empty_array.data(), 20-base_short_string.size()) != 0)
		{
			throw tc::TestException(fmt::format("tc::bn::string<{:d}, {:d}>::encode() did not clear data after string ended", 20, 20));
		}
