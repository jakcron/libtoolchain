...
crypto_Sha2512Generator_TestClass::crypto_Sha2512Generator_TestClass() :
	mTestTag("tc::crypto::Sha2512Generator"),
	mTestResults()
{
}
...
void crypto_Sha2512Generator_TestClass::test_Constants()
{
	...
	try
	{
		...
		{				
			throw tc::TestException(fmt::format("kHashSize had value {:d} (expected {:d})", tc::crypto::Sha2512Generator::kHashSize, kExpectedHashSize));
		}
		...
		{				
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d})", tc::crypto::Sha2512Generator::kBlockSize, kExpectedBlockSize));
		}
		...
		{				
			throw tc::TestException(fmt::format("kAsn1OidData.size() had value {:d} (expected {:d})", tc::crypto::Sha2512Generator::kAsn1OidData.size(), kExpectedAsn1OidData.size()));
		}
		if (memcmp(tc::crypto::Sha2512Generator::kAsn1OidData.data(), kExpectedAsn1OidData.data(), kExpectedAsn1OidData.size()) != 0)
		{
			throw tc::TestException(fmt::format("kAsn1OidData.data() had data {:s} (expected {:s})", tc::cli::FormatUtil::formatBytesAsString(tc::crypto::Sha2512Generator::kAsn1OidData.data(), tc::crypto::Sha2512Generator::kAsn1OidData.size(), true, ""), tc::cli::FormatUtil::formatBytesAsString(kExpectedAsn1OidData, true, "")));
		}
...
void crypto_Sha2512Generator_TestClass::test_UtilFunc()
{
	...
	try
	{
		...
		{
			memset(hash.data(), 0xff, hash.size());
			tc::crypto::GenerateSha2512Hash(hash.data(), (const byte_t*)test_case->in_string.c_str(), test_case->in_string.size());
			if (memcmp(hash.data(), test_case->out_hash.data(), hash.size()) != 0)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed. Had wrong hash value: {:s} (expected {:s})", test_case->test_name, tc::cli::FormatUtil::formatBytesAsString(hash, true, ""), tc::cli::FormatUtil::formatBytesAsString(test_case->out_hash, true, "")));
			}
