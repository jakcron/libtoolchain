...
crypto_HmacSha2256Generator_TestClass::crypto_HmacSha2256Generator_TestClass() :
	mTestTag("tc::crypto::HmacSha2256Generator"),
	mTestResults()
{
}
...
void crypto_HmacSha2256Generator_TestClass::test_Constants()
{
	...
	try
	{
		...
		{				
			throw tc::TestException(fmt::format("kMacSize had value {:d} (expected {:d})", tc::crypto::HmacSha2256Generator::kMacSize, kExpectedMacSize));
		}
		...
		{				
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d})", tc::crypto::HmacSha2256Generator::kBlockSize, kExpectedBlockSize));
		}
...
void crypto_HmacSha2256Generator_TestClass::test_UtilFunc()
{
	...
	try
	{
		...
		{
			memset(mac.data(), 0xff, mac.size());
			tc::crypto::GenerateHmacSha2256Mac(mac.data(), test_case->in_data.data(), test_case->in_data.size(), test_case->in_key.data(), test_case->in_key.size());
			if (memcmp(mac.data(), test_case->out_mac.data(), mac.size()) != 0)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed. Had wrong MAC: {:s} (expected {:s})", test_case->test_name, tc::cli::FormatUtil::formatBytesAsString(mac, true, ""), tc::cli::FormatUtil::formatBytesAsString(test_case->out_mac, true, "")));
			}
...
void crypto_HmacSha2256Generator_TestClass::util_Setup_Rfc2202_TestCases(std::vector<crypto_HmacSha2256Generator_TestClass::TestCase>& test_cases)
{
	TestCase tmp;
