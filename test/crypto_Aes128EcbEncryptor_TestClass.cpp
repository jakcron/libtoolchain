...
crypto_Aes128EcbEncryptor_TestClass::crypto_Aes128EcbEncryptor_TestClass() :
	mTestTag("tc::crypto::Aes128EcbEncryptor"),
	mTestResults()
{
}
...
void crypto_Aes128EcbEncryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes128EcbEncryptor::kBlockSize, kExpectedBlockSize));
		}
		...
		{
			throw tc::TestException(fmt::format("kKeySize had value {:d} (expected {:d})", tc::crypto::Aes128EcbEncryptor::kKeySize, kExpectedKeySize));
		}
...
void crypto_Aes128EcbEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
	...
	try
	{
		...
		// create data
		tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes128EcbEncryptor::kBlockSize).pullData(0, tc::crypto::Aes128EcbEncryptor::kBlockSize);
...
void crypto_Aes128EcbEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
	...
	try
	{
		...
		try
		{
			...
			throw tc::TestException("Failed to throw ArgumentOutOfRangeException where key_size==tc::crypto::Aes128EcbEncryptor::kKeySize-1");
		}
		catch (const tc::ArgumentOutOfRangeException&) { /* do nothing */ }
		catch (const tc::Exception&)
		{
			throw tc::TestException("Failed to throw correct exception where key_size==tc::crypto::Aes128EcbEncryptor::kKeySize-1");
		}
...
void crypto_Aes128EcbEncryptor_TestClass::test_EncryptThrowsExceptionOnBadInput()
{
	...
	try
	{
		...
		{
			throw tc::Exception("Failed to throw correct exception where size==tc::crypto::Aes128EcbEncryptor::kBlockSize-1");
		}
