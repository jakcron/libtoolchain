...
void SourceTestUtil::pullTestHelper(tc::io::ISource& source, int64_t offset, size_t len, size_t expected_len, const byte_t* expected_data)
{
	...
	{
		throw tc::TestException(fmt::format("pullData(offset: {:d}, len: {:d}) returned ByteData with size(): {:d}, when it should have been {:d}.", offset, len, data.size(), expected_len));
	}

	if (expected_data != nullptr && memcmp(data.data(), expected_data, expected_len) != 0)
	{
		throw tc::TestException(fmt::format("pullData(offset: {:d}, len: {:d}) returned ByteData with incorrect layout", offset, len));
	}
}