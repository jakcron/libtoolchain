...
crypto_Des56CbcEncryptor_TestClass::crypto_Des56CbcEncryptor_TestClass() :
	mTestTag("tc::crypto::Des56CbcEncryptor"),
	mTestResults()
{
}
...
void crypto_Des56CbcEncryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d})", tc::crypto::Des56CbcEncryptor::kBlockSize, kExpectedBlockSize));
		}
		...
		{
			throw tc::TestException(fmt::format("kKeySize had value {:d} (expected {:d})", tc::crypto::Des56CbcEncryptor::kKeySize, kExpectedKeySize));
		}
...
void crypto_Des56CbcEncryptor_TestClass::test_UseUtilFuncEnc()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			for (size_t itr = 0; itr < test->iteration_count; itr++)
			{
				tc::crypto::EncryptDes56Cbc(data.data(), data.data(), data.size(), test->key.data(), test->key.size(), iv.data(), iv.size());
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			for (size_t itr = 0; itr < test->iteration_count; itr++)
			{
				...
				tc::crypto::DecryptDes56Cbc(data.data(), data.data(), data.size(), test->key.data(), test->key.size(), iv.data(), iv.size());
...
void crypto_Des56CbcEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
	...
	try
	{
		...
		// create data
		tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Des56CbcEncryptor::kBlockSize).pullData(0, tc::crypto::Des56CbcEncryptor::kBlockSize);
...
void crypto_Des56CbcEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
	...
	try
	{
		...
		try
		{
			cryptor.initialize(tests[0].key.data(), tc::crypto::Des56CbcEncryptor::kKeySize-1, tests[0].iv.data(), tests[0].iv.size());
		...
		try
		{
			cryptor.initialize(tests[0].key.data(), tests[0].key.size(), tests[0].iv.data(), tc::crypto::Des56CbcEncryptor::kBlockSize-1);
