...
crypto_Aes256Encryptor_TestClass::crypto_Aes256Encryptor_TestClass() :
	mTestTag("tc::crypto::Aes256Encryptor"),
	mTestResults()
{
}
...
void crypto_Aes256Encryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes256Encryptor::kBlockSize, kExpectedBlockSize));
		}
		...
		{
			throw tc::TestException(fmt::format("kKeySize had value {:d} (expected {:d})", tc::crypto::Aes256Encryptor::kKeySize, kExpectedKeySize));
		}
...
void crypto_Aes256Encryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
	...
	try
	{
		...
		try
		{
			...
			throw tc::TestException("Failed to throw ArgumentOutOfRangeException where key_size==tc::crypto::Aes256Encryptor::kKeySize-1");
		}
		catch (const tc::ArgumentOutOfRangeException&) { /* do nothing */ }
		catch (const tc::Exception&)
		{
			throw tc::TestException("Failed to throw correct exception where key_size==tc::crypto::Aes256Encryptor::kKeySize-1");
		}
