...
crypto_Pbkdf2Sha512KeyDeriver_TestClass::crypto_Pbkdf2Sha512KeyDeriver_TestClass() :
	mTestTag("tc::crypto::Pbkdf2Sha512KeyDeriver"),
	mTestResults()
{
}
...
const std::string& crypto_Pbkdf2Sha512KeyDeriver_TestClass::getTestTag() const
{
	return mTestTag;
}

const std::vector<ITestClass::TestResult>& crypto_Pbkdf2Sha512KeyDeriver_TestClass::getTestResults() const
{
	return mTestResults;
}
...
void crypto_Pbkdf2Sha512KeyDeriver_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kMaxDerivableSize had value {:d}  (expected {:d})", tc::crypto::Pbkdf2Sha512KeyDeriver::kMaxDerivableSize, kExpectedMaxDerivableSize));
		}
...
void crypto_Pbkdf2Sha512KeyDeriver_TestClass::test_WillThrowExceptionOnZeroRounds()
{
	...
	try
	{
		...
		try 
		{
			...
			throw tc::TestException("DeriveKeyPbkdf2Sha512() Did not throw tc::crypto::CryptoException where the round count was set to zero.");
		}
		catch (const tc::crypto::CryptoException&) { /* do nothing */ }
		catch (const tc::Exception&)
		{
			throw tc::TestException("DeriveKeyPbkdf2Sha512() Threw the wrong exception where the round count was set to zero.");
		}
		...
		try 
		{
			...
			throw tc::TestException("Pbkdf2Sha512KeyDeriver::initialize() Did not throw exception where the round count was set to zero.");
		}
		catch (const tc::crypto::CryptoException&) { /* do nothing */ }
		catch (const tc::Exception&)
		{
			throw tc::TestException("Pbkdf2Sha512KeyDeriver::initialize() Threw the wrong exception where the round count was set to zero.");
		}
...
void crypto_Pbkdf2Sha512KeyDeriver_TestClass::test_WillThrowExceptionOnTooLargeDkSize()
{
	...
	try
	{
		...
		try 
		{
			...
			throw tc::TestException("DeriveKeyPbkdf2Sha512() Did not throw tc::crypto::CryptoException where the derived data length would exceed the maximum.");
		}
		catch (const tc::crypto::CryptoException&) { /* do nothing */ }
		catch (const tc::Exception&)
		{
			throw tc::TestException("DeriveKeyPbkdf2Sha512() Threw the wrong exception where the derived data length would exceed the maximum.");
		}	
		...
		try 
		{
			...
			throw tc::TestException("Pbkdf2Sha512KeyDeriver::getBytes() Did not throw tc::crypto::CryptoException where the derived data length would exceed the maximum.");
		} 
		catch (const tc::crypto::CryptoException&) { /* do nothing */ }
		catch (const tc::Exception&)
		{
			throw tc::TestException("Pbkdf2Sha512KeyDeriver::getBytes() Threw the wrong exception where the derived data length would exceed the maximum.");
		}
