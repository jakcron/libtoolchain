...
crypto_Tdes168CbcEncryptor_TestClass::crypto_Tdes168CbcEncryptor_TestClass() :
	mTestTag("tc::crypto::Tdes168CbcEncryptor"),
	mTestResults()
{
}
...
void crypto_Tdes168CbcEncryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d})", tc::crypto::Tdes168CbcEncryptor::kBlockSize, kExpectedBlockSize));
		}
...
void crypto_Tdes168CbcEncryptor_TestClass::test_UseUtilFuncEnc()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			for (size_t itr = 0; itr < test->iteration_count; itr++)
			{
				tc::crypto::EncryptTdes168Cbc(data.data(), data.data(), data.size(), test->key.data(), test->key.size(), iv.data(), iv.size());
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			for (size_t itr = 0; itr < test->iteration_count; itr++)
			{
				...
				tc::crypto::DecryptTdes168Cbc(data.data(), data.data(), data.size(), test->key.data(), test->key.size(), iv.data(), iv.size());
...
void crypto_Tdes168CbcEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
	...
	try
	{
		...
		// create data
		tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Tdes168CbcEncryptor::kBlockSize).pullData(0, tc::crypto::Tdes168CbcEncryptor::kBlockSize);
...
void crypto_Tdes168CbcEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
	...
	try
	{
		...
		try
		{
			cryptor.initialize(tests[0].key.data(), tc::crypto::Tdes168CbcEncryptor::kKeySize-1, tests[0].iv.data(), tests[0].iv.size());
		...
		try
		{
			cryptor.initialize(tests[0].key.data(), tests[0].key.size(), tests[0].iv.data(), tc::crypto::Tdes168CbcEncryptor::kBlockSize-1);
