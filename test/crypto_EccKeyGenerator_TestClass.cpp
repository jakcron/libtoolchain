...
#if 0
void crypto_EccKeyGenerator_TestClass::test_sbox()
{
	TestResult test_result;
	test_result.test_name = "test_sbox";
	...
	try
	{
		...
		{
			memset(key.d.data(), 0xff, key.d.size());

			memset(key.Q.data(), 0xff, key.Q.size());

			fmt::print("keygen {}\n", i);
			keygen.generateKey(key, tc::crypto::EccCurveType(i));

			fmt::print("d (len: {}): {}\n", key.d.size(), tc::cli::FormatUtil::formatBytesAsString(key.d.data(), key.d.size(), true, ""));
			fmt::print("Q (len: {}): {}\n", key.Q.size(), tc::cli::FormatUtil::formatBytesAsString(key.Q.data(), key.Q.size(), true, ""));
		}
