...
void crypto_Aes192EcbEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
	...
	try
	{
		...
		try
		{
			...
			throw tc::TestException("Failed to throw ArgumentOutOfRangeException where key_size==tc::crypto::Aes192EcbEncryptor::kKeySize-1");
		}
		catch (const tc::ArgumentOutOfRangeException&) { /* do nothing */ }
		catch (const tc::Exception&)
		{
			throw tc::TestException("Failed to throw correct exception where key_size==tc::crypto::Aes192EcbEncryptor::kKeySize-1");
		}
...
void crypto_Aes192EcbEncryptor_TestClass::test_EncryptThrowsExceptionOnBadInput()
{
	...
	try
	{
		...
		{
			throw tc::Exception("Failed to throw correct exception where size==tc::crypto::Aes192EcbEncryptor::kBlockSize-1");
		}
