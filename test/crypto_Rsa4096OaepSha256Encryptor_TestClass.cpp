...
crypto_Rsa4096OaepSha256Encryptor_TestClass::crypto_Rsa4096OaepSha256Encryptor_TestClass() :
	mTestTag("tc::crypto::Rsa4096OaepSha256Encryptor"),
	mTestResults()
{
}
...
const std::string& crypto_Rsa4096OaepSha256Encryptor_TestClass::getTestTag() const
{
	return mTestTag;
}

const std::vector<ITestClass::TestResult>& crypto_Rsa4096OaepSha256Encryptor_TestClass::getTestResults() const
{
	return mTestResults;
}
...
void crypto_Rsa4096OaepSha256Encryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d}", tc::crypto::Rsa4096OaepSha256Encryptor::kBlockSize, kExpectedBlockSize));
		}
...
void crypto_Rsa4096OaepSha256Encryptor_TestClass::test_UseUtilFuncDec()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			if (res != true)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: DecryptRsa4096OaepSha256 returned {} (expected {})", test->test_name, res, true));
			}
			if (message_size != test->dec_message.size())
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: message_size = {:d} (expected {:d})", test->test_name, message_size, test->dec_message.size()));
...
void crypto_Rsa4096OaepSha256Encryptor_TestClass::test_UseUtilFuncEnc()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			if (res != true)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: EncryptRsa4096OaepSha256 returned {} (expected {})", test->test_name, res, true));
			}
