...
crypto_Aes128Encryptor_TestClass::crypto_Aes128Encryptor_TestClass() :
	mTestTag("tc::crypto::Aes128Encryptor"),
	mTestResults()
{
}
...
void crypto_Aes128Encryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes128Encryptor::kBlockSize, kExpectedBlockSize));
		}
		...
		{
			throw tc::TestException(fmt::format("kKeySize had value {:d} (expected {:d})", tc::crypto::Aes128Encryptor::kKeySize, kExpectedKeySize));
		}
...
void crypto_Aes128Encryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
	...
	try
	{
		...
		try
		{
			...
			throw tc::TestException("Failed to throw ArgumentOutOfRangeException where key_size==tc::crypto::Aes128Encryptor::kKeySize-1");
		}
		catch (const tc::ArgumentOutOfRangeException&) { /* do nothing */ }
		catch (const tc::Exception&)
		{
			throw tc::TestException("Failed to throw correct exception where key_size==tc::crypto::Aes128Encryptor::kKeySize-1");
		}
