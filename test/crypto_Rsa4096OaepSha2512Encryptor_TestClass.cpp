...
crypto_Rsa4096OaepSha2512Encryptor_TestClass::crypto_Rsa4096OaepSha2512Encryptor_TestClass() :
	mTestTag("tc::crypto::Rsa4096OaepSha2512Encryptor"),
	mTestResults()
{
}
...
void crypto_Rsa4096OaepSha2512Encryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d}", tc::crypto::Rsa4096OaepSha2512Encryptor::kBlockSize, kExpectedBlockSize));
		}
...
void crypto_Rsa4096OaepSha2512Encryptor_TestClass::test_UseUtilFuncDec()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			// test decryption
			size_t message_size = 0;
			bool res = tc::crypto::DecryptRsa4096OaepSha2512(data.data(), message_size, data.size(), test->enc_message.data(), tc::crypto::RsaPrivateKey(test->key_modulus.data(), test->key_modulus.size(), test->key_private_exponent.data(), test->key_private_exponent.size()), test->label.data(), test->label.size(), test->label_is_digested);
			if (res != true)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: DecryptRsa4096OaepSha2512 returned {} (expected {})", test->test_name, res, true));
			}
			if (message_size != test->dec_message.size())
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: message_size = {:d} (expected {:d})", test->test_name, message_size, test->dec_message.size()));
...
void crypto_Rsa4096OaepSha2512Encryptor_TestClass::test_UseUtilFuncEnc()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			// test encryption
			bool res = tc::crypto::EncryptRsa4096OaepSha2512(enc_data.data(), test->dec_message.data(), test->dec_message.size(), tc::crypto::RsaPublicKey(test->key_modulus.data(), test->key_modulus.size()), test->label.data(), test->label.size(), test->label_is_digested);
			if (res != true)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: EncryptRsa4096OaepSha2512 returned {} (expected {})", test->test_name, res, true));
			}

			// try to decrypt message
			size_t message_size = 0;
			res = tc::crypto::DecryptRsa4096OaepSha2512(dec_data.data(), message_size, dec_data.size(), enc_data.data(), tc::crypto::RsaPrivateKey(test->key_modulus.data(), test->key_modulus.size(), test->key_private_exponent.data(), test->key_private_exponent.size()), test->label.data(), test->label.size(), test->label_is_digested);
			if (res != true)
			{
...
void crypto_Rsa4096OaepSha2512Encryptor_TestClass::test_DoesNothingWhenNotInit()
{
	...
	{	
		...
		tc::ByteData control_block = tc::io::PaddingSource(0xdd, tc::crypto::Rsa4096OaepSha2512Encryptor::kBlockSize).pullData(0, tc::crypto::Rsa4096OaepSha2512Encryptor::kBlockSize);
...
void crypto_Rsa4096OaepSha2512Encryptor_TestClass::test_EncryptReturnsFalseOnBadInput()
{
	...
	try
	{
		...
		size_t max_message_size = tc::crypto::Rsa4096OaepSha2512Encryptor::kBlockSize - (2 * tc::crypto::Sha2512Generator::kHashSize) - 2;
