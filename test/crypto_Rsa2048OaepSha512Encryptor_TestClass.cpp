...
crypto_Rsa2048OaepSha512Encryptor_TestClass::crypto_Rsa2048OaepSha512Encryptor_TestClass() :
	mTestTag("tc::crypto::Rsa2048OaepSha512Encryptor"),
	mTestResults()
{
}
...
const std::string& crypto_Rsa2048OaepSha512Encryptor_TestClass::getTestTag() const
{
	return mTestTag;
}

const std::vector<ITestClass::TestResult>& crypto_Rsa2048OaepSha512Encryptor_TestClass::getTestResults() const
{
	return mTestResults;
}
...
void crypto_Rsa2048OaepSha512Encryptor_TestClass::test_Constants()
{
	...
	try
	{
		...
		{
			throw tc::TestException(fmt::format("kBlockSize had value {:d} (expected {:d}", tc::crypto::Rsa2048OaepSha512Encryptor::kBlockSize, kExpectedBlockSize));
		}
...
void crypto_Rsa2048OaepSha512Encryptor_TestClass::test_UseUtilFuncDec()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			if (res != true)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: DecryptRsa2048OaepSha512 returned {} (expected {})", test->test_name, res, true));
			}
			if (message_size != test->dec_message.size())
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: message_size = {:d} (expected {:d})", test->test_name, message_size, test->dec_message.size()));
...
void crypto_Rsa2048OaepSha512Encryptor_TestClass::test_UseUtilFuncEnc()
{
	...
	try
	{
		...
		for (auto test = tests.begin(); test != tests.end(); test++)
		{
			...
			if (res != true)
			{
				throw tc::TestException(fmt::format("Test \"{:s}\" Failed: EncryptRsa2048OaepSha512 returned {} (expected {})", test->test_name, res, true));
			}
