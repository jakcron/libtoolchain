...
namespace tc { namespace crypto {
	...
	/**
	 * @typedef Rsa1024OaepSha512Encryptor
	 * @brief Class for generating and verifying RSA1024-OAEP-SHA2-512 signatures.
	 * 
	 * @details This class uses RSA1024-OAEP to sign/validate SHA2-512 message digests.
	 * For more information refer to @ref RsaOaepEncryptor.
	 */
using Rsa1024OaepSha512Encryptor = RsaOaepEncryptor<1024,Sha512Generator>;
...
using Rsa4096OaepSha512Encryptor = RsaOaepEncryptor<4096,Sha512Generator>;
	...
	/**
	 * @brief Utility function for encrypting a message using RSA1024-OAEP-SHA2-512.
	 * 
	 ...
	 * 
	 * @pre
	 * - Size of the @p block buffer must >= <tt>Rsa1024OaepSha512Encryptor::kBlockSize</tt>.
	 * - The maximum size for @p message_size is <tt>Rsa1024OaepSha512Encryptor::kBlockSize</tt> - (2 * <tt>Sha512Generator::kHashSize</tt>) - 2
	 * 
	 * @post
	 ...
	 * OAEP encoding uses a random seed, this function generates the seed internally. To manually specify the seed, please use the  @ref Rsa1024OaepSha512Encryptor class directly.
	 */
bool EncryptRsa1024OaepSha512(byte_t* block, const byte_t* message, size_t message_size, const RsaKey& key, const byte_t* label, size_t label_size, bool isLabelDigested = false);

	/**
	 * @brief Utility for decrypting a RSA1024-OAEP-SHA2-512 encrypted message.
	 * 
	 ...
	 * @pre
	 * - Size of the @p block buffer must >= <tt>Rsa1024OaepSha512Encryptor::kBlockSize</tt>.
	 * - @p message_capacity >= (<tt>Rsa1024OaepSha512Encryptor::kBlockSize</tt> - (2 * <tt>Sha512Generator::kHashSize</tt>) - 2)
	 * 
	 * @post
	 ...
	 * OAEP encoding uses a label (which is digested using a hash algorithm) as part of the MGF1 masking process. It is possible to specify a pre-digested label instead, in which case set @p isLabelDigested to true.
	 */
bool DecryptRsa1024OaepSha512(byte_t* message, size_t& message_size, size_t message_capacity, const byte_t* block, const RsaKey& key, const byte_t* label, size_t label_size, bool isLabelDigested = false);
