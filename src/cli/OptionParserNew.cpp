#include <tc/cli/OptionParserNew.h>

#include <iostream>
#include <sstream>

tc::cli::OptionParserNew::OptionParserNew() :
	mModuleLabel("OptionParserNew"),
	mOptions(),
	mUnkOptHandler(nullptr)
{
}

void tc::cli::OptionParserNew::registerOptionHandler(const std::shared_ptr<IOptionHandler>& handler)
{
	if (handler == nullptr)
	{
		// throw exception
...
void tc::cli::OptionParserNew::registerUnrecognisedOptionHandler(const std::shared_ptr<IOptionHandler>& handler)
{
	if (handler == nullptr)
	{
		// throw exception
...
void tc::cli::OptionParserNew::processOptions(const std::vector<std::string>& args)
{
	/*
	std::cout << "OptionParserNew" << std::endl;
...
void tc::cli::OptionParserNew::processOptions(const std::vector<std::string>& args, size_t pos, size_t num)
{
	processOptions({args.begin()+pos, args.begin()+pos+num});
}

void tc::cli::OptionParserNew::handleOption(const std::string& opt, const std::vector<std::string>& params)
{
