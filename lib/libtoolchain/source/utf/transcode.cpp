#include <tc/utf/transcode.h>
#include <tc/Exception.h>

void tc::utf::transcodeUTF8ToUTF32(const std::string& src, std::u32string& dst)
{
	size_t done = 0;
	dst.clear();
...
void tc::utf::transcodeUTF16ToUTF32(const std::u16string& src, std::u32string& dst)
{
	size_t done = 0;
	dst.clear();
	for (size_t i = 0; i < src.length(); i+=done)
...
void tc::utf::transcodeUTF32ToUTF8(const std::u32string& src, std::string& dst)
{
	dst.clear();
...
void tc::utf::transcodeUTF32ToUTF16(const std::u32string& src, std::u16string& dst)
{
	dst.clear();
...
void tc::utf::transcodeUTF8ToUTF16(const std::string& src, std::u16string& dst)
{
	std::u32string unicode;
	transcodeUTF8ToUTF32(src, unicode);
	transcodeUTF32ToUTF16(unicode, dst);
	/*
	size_t done = 0;
	for (size_t i = 0; i < src.length(); i += done)
	{
...
void tc::utf::transcodeUTF16ToUTF8(const std::u16string& src, std::string& dst)
{
	std::u32string unicode;
	transcodeUTF16ToUTF32(src, unicode);
	transcodeUTF32ToUTF8(unicode, dst);
	/*
	size_t done = 0;
	for (size_t i = 0; i < src.length(); i+=done)
	{
